...
&GetOptions('mode=s', \$sc_v{'mode'}, 'guild', \$sc_v{'guild'}, 'guildname=s', \$sc_v{'guildname'});

#$sc_v{'guild'} = $sc_v{'guild'}?1:0;
$sc_v{'guild'} = 0 if (!$sc_v{'guild'});
$sc_v{'mode'} = $sc_v{'mode'}?1:0;
...
print <<EOM;
	...
	Regkey		$version{'regkey'}
	Chkkey		$version{'chkkey'}

	checkVer	$sc_v{'mode'}

	Guild		$sc_v{'guild'}
	Guild Check	$sc_v{'guildname'}
...
\$sc_v\{'Scorpio'\}\{'regkey'\}	= "$version{'regkey'}";

\$sc_v\{'Scorpio'\}\{'checkVer'\}	= $sc_v{'mode'};

\$sc_v\{'Scorpio'\}\{'checkUser'\}	= $sc_v{'guild'};
\$sc_v\{'Scorpio'\}\{'checkGuild'\}	= \'$sc_v{'guildname'}\';
...
%make = (
	...
	, InternalName => '"Scorpio"'
	, OriginalFilename => '"Scorpio"'
	, ProductName => '"Scorpio"'
